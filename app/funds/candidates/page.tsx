...
const gradeLinks = [
  ...
  { key: "strong", label: "重点观察", note: "观察分 75 以上", href: "/funds/candidates?grade=strong" },
  { key: "watchable", label: "可以观察", note: "观察分 60-74", href: "/funds/candidates?grade=watchable" },
  { key: "cautious", label: "谨慎观察", note: "数据或风险仍需确认", href: "/funds/candidates?grade=cautious" }
];
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        ...
        <Link href="/funds?view=watchlist" className="focus-ring rounded-md border border-line bg-white px-3 py-2 text-sm text-steel">
          查看我的关注
        </Link>
      </div>
      ...
      <form className="grid gap-3 rounded-md border border-line bg-white p-4 shadow-panel md:grid-cols-[1fr_160px_160px_100px]">
        {params.grade !== "all" ? <input type="hidden" name="grade" value={params.grade} /> : null}
        <input
          name="type"
          defaultValue={params.type}
          placeholder="基金类型：混合型 / 债券型 / ETF"
                ...
                result.items.map((item) => (
                  <tr key={item.code} className="hover:bg-paper/70">
                    <td className="px-4 py-3">
                      ...
                      <div className="mt-1 text-xs text-slate-500">
                        样本 {item.navSampleCount} 日，数据分 {item.dataScore}
                      </div>
                    </td>
